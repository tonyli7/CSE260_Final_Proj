...
public abstract class Sprite implements Collideable{
    ...
    protected ArrayList<Integer> dir_pattern;
    protected int pattern_index;
	...
	dir_pattern = new ArrayList<Integer>();
	pattern_index = 0;
	steps = 0;
	form = 0;
	curr_img = down_imgs[0];
	height = curr_img.getHeight()*1.5;
	width = curr_img.getWidth()*1.5;
	...
	if (dir_pattern.size() > 0){
	    pattern_index += 1;
	    pattern_index %= dir_pattern.size();
	    dir = dir_pattern.get(pattern_index);
	}
    }

    public void setDirPattern(ArrayList<Integer> dir_pat){
	dir_pattern = dir_pat;
    }
